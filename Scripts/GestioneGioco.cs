...
public class GestioneGioco : MonoBehaviour
{
    ...
    public void SettaOggetto(GameObject torretta)
    {
        ...
        testoDescrizione.text = torretta.GetComponent<Torretta>().descrizione;
        testoDanno.text = "Danno: "+torretta.GetComponent<Torretta>().danno;
        testoAttacchiPerSecondo.text = "Attacchi/sec: " + torretta.GetComponent<Torretta>().attacchiPerSecondo;
        testoRaggio.text = "Raggio: " + torretta.GetComponent<Torretta>().raggio;
    ...
    private void PressioneMouse()
    {
		...
		if (hit.collider.tag=="Casella" && torretta!=null && hit.collider.GetComponent<Casella>().occupata == false && risorse >= torretta.GetComponent<Torretta>().costo)
        {
            Instantiate(torretta, hit.collider.transform.position + Vector3.up, new Quaternion());
            risorse -= torretta.GetComponent<Torretta>().costo;
